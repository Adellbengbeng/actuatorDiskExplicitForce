...
namespace Foam {
    ...
    void actuatorDiskExplicitForce::ReadGeometry(const fvMesh &iMesh) {
        ...
        Istream& is5 = iMesh.solutionDict().subDict("actuatorDisk").lookup("density");
        is5.format(IOstream::ASCII);
        is5 >> mRho;

        Istream& is6 = iMesh.solutionDict().subDict("actuatorDisk").lookup("startPoint");
        is6.format(IOstream::ASCII);
        is6 >> mPointStartCenterLine;

        Istream& is7 = iMesh.solutionDict().subDict("actuatorDisk").lookup("endPoint");
